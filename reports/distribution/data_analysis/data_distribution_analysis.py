...
PROJECT_DIR = next(
    (parent for parent in Path(__file__).resolve().parents if (parent / "pixi.toml").exists()),
    Path.cwd(),
)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Input CSV file path. Default: data/processed/data_vectorized.csv",
    )
    parser.add_argument(
        "--output-dir",
        default="reports/distribution/distribution_analysis",
        help="Output directory. Default: reports/distribution/distribution_analysis",
    )
    parser.add_argument(
        "--max-categories",
        type=int,
...
def main():
    ...
    input_path = input_arg if input_arg.is_absolute() else (PROJECT_DIR / input_arg).resolve()
    output_dir = output_arg if output_arg.is_absolute() else (PROJECT_DIR / output_arg).resolve()
    ...
    readme.write_text(
        "\n".join(
            [
                ...
                f"- Frequency tables: `{display_path(freq_dir)}`",
                f"- Plots: `{display_path(plot_dir)}`",
                f"- Histogram tables: `{display_path(hist_table_dir)}`",
