...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional reference split CSV. When provided, it overrides --test-size/--val-size splitting.",
    )
...
def run_one_target(args, df: pd.DataFrame, target_col: str, output_dir: Path):
    ...
    if args.split_file is not None:
        ...
        train_all_pos, test_pos = split_train_test_stratified(y_bin, args.test_size, args.random_state)
        tr_sub_idx, val_sub_idx = split_train_val_stratified(y_bin[train_all_pos], args.val_size, args.random_state + 17)
        train_pos = train_all_pos[tr_sub_idx]
        val_pos = train_all_pos[val_sub_idx]

    x_train, x_val, x_test = x[train_pos], x[val_pos], x[test_pos]
    y_train, y_val, y_test = y_bin[train_pos], y_bin[val_pos], y_bin[test_pos]
    score_train = y_score[train_pos]
