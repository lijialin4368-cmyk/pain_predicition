...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            "then map predicted scores to high-pain probability for final 0/1 decision."
        )
    )
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=script_dir / "3_output_prediction_report_en_scoreprob",
...
def compute_score_element_grad(y_score: np.ndarray, pred_score: np.ndarray, unit_prob: np.ndarray, mask: np.ndarray, args):
    ...
    dlogits = grad_score * (PAIN_SCORE_MAX * unit_prob * (1.0 - unit_prob))
    dlogits[~mask] = 0.0
    loss_elem[~mask] = 0.0
    return loss_elem, dlogits
...
def choose_threshold_conservative(args, target_col: str, y_val: np.ndarray, prob_val: np.ndarray):
    ...
    max_shift = max(0.0, float(args.conservative_max_shift))
    candidates = np.array([
    ...
    feasible = []
    recall_floor = max(0.0, base["recall"] - float(args.conservative_max_recall_drop))
    f1_floor = base["f1"] + float(args.conservative_min_f1_delta)
    acc_floor = base["accuracy"] + float(args.conservative_min_acc_gain)
...
def fit_shared_backbone_score_multitask(
    ...
    for epoch in range(1, int(args.epochs) + 1):
        ...
        for start in range(0, n_samples, batch_size):
            ...
            loss_elem, dlogits = compute_score_element_grad(yb, score_pred, unit_prob, mb, args)
        ...
        train_loss = weighted_score_loss(y_filled, score_all, eval_weight, args.loss_type, args.huber_delta)
...
def main():
    ...
    print("Training target          : raw pain score regression (0-10), pain-only")
