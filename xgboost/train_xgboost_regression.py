...
try:
    ...
    raise ImportError("缺少 xgboost 依赖，请先执行 `pixi install` 或更新 pixi 环境。") from exc
...
def parse_args() -> argparse.Namespace:
    ...
    augmented_data_path = project_dir / "data_augmentation" / "generated" / "augmented_dataset.csv"
    default_data_path = augmented_data_path if augmented_data_path.exists() else DATA_PATH
    parser = argparse.ArgumentParser(description="Train and evaluate an XGBoost regressor.")
    parser.add_argument(
        "--data-path",
        type=Path,
        default=default_data_path,
        help="Input CSV path. Defaults to augmented dataset when available, otherwise config.DATA_PATH.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
...
def build_xgb_pipeline(xgb_params: dict) -> Pipeline:
    return Pipeline(
        steps=[
...
def run_hyperparameter_search(X_train: pd.DataFrame, y_train: pd.Series) -> tuple[Pipeline, dict, pd.DataFrame]:
    search_xgb_params = dict(XGB_PARAMS)
    search_xgb_params["n_jobs"] = 1
    pipeline = build_xgb_pipeline(search_xgb_params)
    cv = KFold(n_splits=XGB_SEARCH_CV, shuffle=True, random_state=RANDOM_STATE)
    param_distributions = {f"model__{k}": v for k, v in XGB_SEARCH_PARAM_DISTRIBUTIONS.items()}
    search = RandomizedSearchCV(
        estimator=pipeline,
        param_distributions=param_distributions,
        n_iter=XGB_SEARCH_N_ITER,
        scoring=XGB_SEARCH_SCORING,
        cv=cv,
    ...
    search_summary = {
        "enabled": True,
        "n_iter": int(XGB_SEARCH_N_ITER),
        "cv_splits": int(XGB_SEARCH_CV),
        "scoring": XGB_SEARCH_SCORING,
...
def train_and_evaluate(
    ...
    if USE_HYPERPARAM_SEARCH:
        ...
        search_summary = {
            "enabled": False,
            "best_params": {k: to_jsonable_scalar(v) for k, v in XGB_PARAMS.items()},
        }
        cv_results_df = None
...
def save_outputs(
    ...
    if cv_results_df is not None:
        cv_results_path = output_dir / "xgb_search_results.csv"
        keep_cols = [
            "rank_test_score",
            "mean_test_score",
            "std_test_score",
    ...
    if cv_results_df is not None:
        print(f"调参结果: {output_dir / 'xgb_search_results.csv'}")
