...
DEFAULT_OUTPUT_DIR = BASE_DIR / "outputs" / "raw"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Visualize XGBoost regression outputs.")
    parser.add_argument(
        "--output-dir",
        type=Path,
...
def plot_abs_error_by_true(pred_df: pd.DataFrame, plot_dir: Path) -> Path:
    group = pred_df.groupby("y_true", as_index=False)["abs_error"].mean().sort_values("y_true")
...
def plot_feature_importance(fi_df: pd.DataFrame, plot_dir: Path, top_n: int = 20) -> tuple[Path, Path]:
    top_df = fi_df.sort_values("importance", ascending=False).head(top_n).reset_index(drop=True)
    ...
    top_df[["feature_en", "feature", "importance"]].to_csv(mapping_path, index=False, encoding="utf-8-sig")
