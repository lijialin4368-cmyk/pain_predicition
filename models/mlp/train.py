...
from splits.split_utils import (
    ...
    get_target_tier,
    split_positions_from_reference,
)
...
def parse_args():
    ...
    project_dir = PROJECT_DIR or script_dir.parents[1]

    default_input_candidates = [
        project_dir / "data" / "processed" / "data_vectorized.csv",
    ]
    ...
    parser.add_argument("--test-size", type=float, default=0.1, help="Test set ratio.")
    parser.add_argument(
        "--split-file",
        type=Path,
        default=None,
        help=(
            "Optional reference split CSV. Shared-backbone mode reads the "
            f"{SHARED_BACKBONE_SPLIT_TARGET} rows from this file."
        ),
    )
    ...
    parser.add_argument(
        "--threshold-strategy",
        type=str,
        choices=["fixed", "tune", "day_relaxed"],
        default="day_relaxed",
        help=(
    ...
    parser.add_argument("--val-size", type=float, default=1.0 / 9.0, help="Validation split from train pool; default gives total train/val/test ~= 80/10/10.")
...
def fit_preprocessor(x_train: np.ndarray, feature_impute: str):
    ...
    x_filled = np.where(np.isfinite(x), x, fill_values.reshape(1, -1))
    mean = x_filled.mean(axis=0)
    std = x_filled.std(axis=0)
    mean = np.where(np.isfinite(mean), mean, 0.0)
    std = np.where((std > 0.0) & np.isfinite(std), std, 1.0)
    return {"fill_values": fill_values.astype(float), "mean": mean.astype(float), "std": std.astype(float)}


def transform_preprocessor(x: np.ndarray, preprocessor: dict) -> np.ndarray:
    x_arr = np.asarray(x, dtype=float)
    if x_arr.size == 0:
        return x_arr.reshape((0, len(preprocessor["fill_values"])))
    fill_values = preprocessor["fill_values"].reshape(1, -1)
    x_filled = np.where(np.isfinite(x_arr), x_arr, fill_values)
    return (x_filled - preprocessor["mean"].reshape(1, -1)) / preprocessor["std"].reshape(1, -1)
...
def compute_average_precision(y_true: np.ndarray, y_prob: np.ndarray):
    y_true = y_true.astype(int)
    pos = int(np.sum(y_true == 1))
    if pos == 0:
        return float("nan")
    ...
    return float(np.sum(precision_at_k[y_sorted == 1]) / pos)
...
def classification_metrics(y_true: np.ndarray, y_prob: np.ndarray, threshold: float = 0.5):
    ...
    auprc_lift = float(auprc / positive_rate) if positive_rate > 0.0 and np.isfinite(auprc) else float("nan")
        ...
        rows.append(
            {
                ...
                "auc": float(m["auc"]),
                "auprc": float(m["auprc"]),
                "positive_rate": float(m["positive_rate"]),
                "auprc_lift": float(m["auprc_lift"]),
...
def run_shared_backbone_targets(args, df: pd.DataFrame, targets):
    ...
    row_ids = df.index[any_label_mask].to_numpy(dtype=int)
    ...
    if args.split_file is not None:
        ...
        train_all_idx, test_idx = split_train_test_multitask(y_bin_mat, args.test_size, args.random_state)
        tr_sub_idx, val_sub_idx = split_train_val_multitask(y_bin_mat[train_all_idx], args.val_size, args.random_state + 17)
        train_idx = train_all_idx[tr_sub_idx]
        val_idx = train_all_idx[val_sub_idx]

    x_train, x_val, x_test = x_all[train_idx], x_all[val_idx], x_all[test_idx]
    y_train, y_val, y_test = y_bin_mat[train_idx], y_bin_mat[val_idx], y_bin_mat[test_idx]
    score_train = y_score_mat[train_idx]

    preprocessor = fit_preprocessor(x_train, args.feature_impute)
    x_train_std = transform_preprocessor(x_train, preprocessor)
    x_val_std = transform_preprocessor(x_val, preprocessor)
    x_test_std = transform_preprocessor(x_test, preprocessor)
...
def run_one_target(args, df: pd.DataFrame, target_col: str, output_dir: Path):
    ...
    row_ids = df.index[valid_mask].to_numpy(dtype=int)
    ...
    if args.split_file is not None:
        ...
        train_all_idx, test_idx = split_train_test_stratified(y_bin, args.test_size, args.random_state)
        tr_sub_idx, val_sub_idx = split_train_val_stratified(y_bin[train_all_idx], args.val_size, args.random_state + 17)
        train_idx = train_all_idx[tr_sub_idx]
        val_idx = train_all_idx[val_sub_idx]
...
def print_result(result: dict):
    ...
    print(f"Positive Rate            : {m['positive_rate']:.4f}")
    print(f"AUPRC Lift               : {m['auprc_lift']:.4f}")
...
def main():
    args = parse_args()
    args.day_threshold_map = parse_day_thresholds(args.day_thresholds) if args.threshold_strategy == "day_relaxed" else {}
