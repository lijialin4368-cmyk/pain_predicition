"""Train single-target linear regression baselines."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a linear regression baseline.")
    parser.add_argument("--data-path", type=Path, default=DATA_PATH)
    parser.add_argument("--output-dir", type=Path, default=OUTPUT_DIR)
    parser.add_argument("--artifact-dir", type=Path, default=ARTIFACT_DIR)
    parser.add_argument("--split-file", type=Path, default=None)
    parser.add_argument("--split-seed", type=int, default=42)
    parser.add_argument("--model", choices=["ridge", "elasticnet"], default="ridge")
...
def build_pipeline(model_name: str) -> tuple[Pipeline, dict]:
    if model_name == "ridge":
        ...
        param_grid = {"model__alpha": ELASTICNET_ALPHAS, "model__l1_ratio": ELASTICNET_L1_RATIOS}
    pipeline = Pipeline(
        steps=[
...
def main() -> None:
    ...
    train_idx, val_idx, test_idx = make_split(X, is_generated, split_labels, args.split_file, args.split_seed)
    X_train = X.loc[train_idx]
    y_train = y.loc[train_idx]
    X_test = X.loc[test_idx]
    y_test = y.loc[test_idx]

    pipeline, param_grid = build_pipeline(args.model)
    ...
    model_path = args.artifact_dir / f"{args.model}_regressor.joblib"
    ...
    coef_df.sort_values("abs_coefficient", ascending=False).to_csv(
        args.output_dir / "coefficients.csv", index=False, encoding="utf-8-sig"
    )
    cv_results = pd.DataFrame(search.cv_results_).sort_values("rank_test_score")
    keep_cols = [col for col in cv_results.columns if col.startswith("param_") or col in {"rank_test_score", "mean_test_score", "std_test_score", "mean_train_score", "std_train_score"}]
    cv_results.loc[:, keep_cols].to_csv(args.output_dir / "linear_search_results.csv", index=False, encoding="utf-8-sig")
    ...
    print(f"系数表: {args.output_dir / 'coefficients.csv'}")
