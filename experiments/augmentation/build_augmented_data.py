...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=DATA_PATH,
        help="Input CSV path. Defaults to experiments/augmentation/config.py::DATA_PATH.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Fraction of original rows reserved as validation originals before augmentation.",
    )
    parser.add_argument(
        "--test-size",
        type=float,
        default=TEST_SIZE,
        help="Fraction of original rows reserved as test originals before augmentation.",
    )
    parser.add_argument(
        "--random-state",
        type=int,
...
def split_source_dataset(
    ...
    *,
    ...
    if validation_size < 0 or test_size < 0:
        raise ValueError("--validation-size 和 --test-size 不能为负数。")

    holdout_size = validation_size + test_size
    if holdout_size <= 0 or holdout_size >= 1:
        raise ValueError("--validation-size + --test-size 必须在 (0, 1) 范围内。")
    ...
    if validation_size == 0:
        return pd.Index(train_idx), pd.Index([]), pd.Index(holdout_idx)
    if test_size == 0:
        return pd.Index(train_idx), pd.Index(holdout_idx), pd.Index([])

    test_fraction_within_holdout = test_size / holdout_size
    ...
    return pd.Index(train_idx), pd.Index(validation_idx), pd.Index(test_idx)
...
def build_summary(
    ...
    return {
        ...
        "heldout_size_fraction": float(validation_size + test_size),
        "validation_size_fraction": float(validation_size),
        ...
        "heldout_original_rows": int(len(validation_original_df) + len(test_original_df)),
        "test_original_rows": int(len(test_original_df)),
        "generated_rows": int(len(generated_df)),
        "final_rows": int(len(train_original_df) + len(validation_original_df) + len(test_original_df) + len(generated_df)),
...
def main() -> None:
    ...
    if not (0 <= args.validation_size < 1):
        raise ValueError("--validation-size 必须在 [0, 1) 之间。")
    if not (0 <= args.test_size < 1):
        raise ValueError("--test-size 必须在 [0, 1) 之间。")

    source_df = load_source_data(args.data_path)
    train_idx, validation_idx, test_idx = split_source_dataset(
        source_df,
    ...
    validation_source_df = source_df.loc[validation_idx].copy()
    ...
    validation_original_df = attach_original_metadata(validation_source_df, VALIDATION_SPLIT_VALUE)
    ...
    validation_original_df = validation_original_df.reindex(columns=combined_columns)
    test_original_df = test_original_df.reindex(columns=combined_columns)
    combined_df = pd.concat(
        [train_original_df, generated_df, validation_original_df, test_original_df],
        ignore_index=True,
    )
    ...
    print(f"测试原始样本: {len(test_original_df)}")
